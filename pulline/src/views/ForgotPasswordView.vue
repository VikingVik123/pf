    ...
    <main class="form-side">
      <div class="card">
        ...
        <template v-if="!submitted">
          ...
          <p class="subhead">Enter your email and we'll send you a link to reset it.</p>
        ...
        <template v-else>
          ...
          <p class="subhead">
            If an account exists for <strong>{{ form.email }}</strong>, we've sent instructions to reset your password.
          </p>
        </template>
...
async function handleSubmit() {
  ...
  try {
    ...
    if (err.response?.status === 422) {
      ...
      // by defaulting to the success state rather than an error banner.
      submitted.value = true
    }
  } finally {
